...
use nft_simple::Token;
use near_sdk::json_types::{U64, U128};
use near_sdk::serde_json::json;
use near_sdk_sim::{
    init_simulator, to_yocto, UserAccount, DEFAULT_GAS, STORAGE_AMOUNT,
};
...
fn simulate_approval_workflow() {
    ...
    let mut token_info: Token = root_account.view(
        nft.account_id(),
        "nft_token",
            &json!({
                "token_id": "0"
    ...
    assert_eq!(token_info.approved_account_ids.len(), 2, "Expected two approvers.");
    assert_eq!(token_info.approval_counter, U64::from(2), "Expected two approvers.");
    ...
    alice.call(
        ...
        &json!({
            "receiver_id": alice.account_id(),
            "token_id": "0".to_string(),
            "enforce_approval_id": "1".to_string(),
    ...
    assert_eq!(token_info.approved_account_ids.len(), 0, "Expected no approvers after a transfer.");
    ...
    let outcome = bob.call(
        ...
        &json!({
            "receiver_id": bob.account_id(),
            "token_id": "0".to_string(),
            "enforce_approval_id": "2".to_string(),
            "memo": "glad I got this before Bob"
        }).to_string().into_bytes(),
        DEFAULT_GAS,
        1, // deposit
    ...
    if let ExecutionStatus::Failure(err) = status {
        ...
        assert_eq!(err.to_string(), "Action #0: Smart contract panicked: Unauthorized");
    } else {
...
fn simulate_exquisite_corpse_interactions() {
    ...
    assert_eq!(token_info.owner_id, ec.account_id(), "Expected ec account to be the new owner.");
    ...
    let mut exquisite_corpses: Vec<ExquisiteCorpse> = nft.view(
        ec.account_id(),
        "show_all_corpses",
        &[]
    ).unwrap_json();
    ...
    let mut can_manually_create_ec: bool = root_account.view(
        ec.account_id(),
        "can_create_corpse_from_approvals",
        &[]
    ).unwrap_json();
    ...
    can_manually_create_ec = root_account.view(
        ec.account_id(),
        "can_create_corpse_from_approvals",
        &[]
    ).unwrap_json();
    assert!(can_manually_create_ec, "Should be able to manually create an exquisite corpse once all pieces have been approved.");
    ...
    /*
        ...
        msg: String,
     */
    let num_tokens_used: U128 = alice.call(
        ft.account_id(),
        "ft_transfer_call",
        &json!({
            "receiver_id": ec.account_id(),
...
fn sim_helper_init() -> (UserAccount, UserAccount) {
    ...
    genesis.runtime_config.transaction_costs.action_creation_config.function_call_cost.execution = 0u64;
